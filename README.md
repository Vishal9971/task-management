...
6. express-rate-limit – API rate limiting
...
4. Check versions:
   a. node -v
   b. npm -v
   2️⃣ Clone the Repository 1. git clone https://github.com/Vishal9971/task-management-api.git 2. cd task-management-api
   3️⃣ Install Dependencies 1. npm install
   4️⃣ Environment Variables Configuration
   ~ Create a .env file in the root directory: 1. PORT=3001 2. MONGO_URI=mongodb://localhost:27017/task_management 3. JWT_SECRET=your_jwt_secret_key
   ⚠️ Note: .env is ignored using .gitignore for security reasons.
   5️⃣ Start the Server 1. npm start
   Expected output:
   ~ Server running on port 3001
   ~ MongoDB Connected

🔐 Authentication Flow 1. Register → Create a user with role 2. Login → Receive JWT token 3. Pass token → Authorization header 4. Access protected routes
Header format:
~ Authorization: Bearer <JWT_TOKEN>

👥 Roles & Permissions 1. Admin Full access (users, tasks, analytics) 2. Manager Assign tasks, manage team tasks 3. User Manage own tasks & profile
...
~ Users receive live notifications when: 1. Task is created 2. Task is updated 3. Task is assigned
